...
Status DBImpl::ForceFullCompaction() {
  ...
  {
    MutexLock l(&mutex_);
    // Don't let two ForceFullCompaction calls run at once.
    // They share force_full_compaction_stats_, so overlapping calls could
    // clobber each other's stats or leave a bad pointer behind.
  ...
  Log(options_.info_log,
      "Manual full compaction complete:\n"
      "  Compactions executed: %lld\n"
      "  Input files:         %lld\n"
      "  Output files:        %lld\n"
      "  Bytes read:          %lld\n"
      "  Bytes written:       %lld\n",
      static_cast<long long>(full_compaction_stats.compactions),
      static_cast<long long>(full_compaction_stats.input_files),
...
Status DBImpl::DoCompactionWork(CompactionState* compact) {
  ...
  if (force_full_compaction_stats_ != nullptr) {
    ...
    force_full_compaction_stats_->input_files +=
        ...
        compact->compaction->num_input_files(1);
    force_full_compaction_stats_->output_files += compact->outputs.size();
    force_full_compaction_stats_->bytes_read += stats.bytes_read;
    force_full_compaction_stats_->bytes_written += stats.bytes_written;
  }
...
Status DBImpl::Get(const ReadOptions& options, const Slice& key,
  ...
  Status s;
  MutexLock l(&mutex_);
  // Pause reads while ForceFullCompaction is running.
  // The database should be briefly unavailable for foreground work during
...
Status DBImpl::Scan(const ReadOptions& options, const Slice& start_key,
  ...
  // Only create a snapshot when the caller did not supply one.
  // Otherwise we might read writes that happened after the caller's
  // intended view of the database.
...
Status DBImpl::DeleteRange(const WriteOptions& write_options,
  ...
  // Enqueue as an exclusive writer so no Put can slip in between the scan and
  // the commit. Once we reach the front, the writers behind us stay blocked
  // until we pop ourselves.
  Writer w(&mutex_);
  w.batch = &batch;
  ...
  // We're at the front. Release the mutex so NewIterator can grab it briefly,
  // but new writers still can't run.
  mutex_.Unlock();
...
Status DBImpl::Write(const WriteOptions& options, WriteBatch* updates) {
  ...
  MutexLock l(&mutex_);
  // Pause external writes while ForceFullCompaction is running.
  // We still allow updates == nullptr so TEST_CompactMemTable() can flush
  // the memtable without deadlocking.
  if (updates != nullptr) {
    while (force_compaction_in_progress_) {
      force_compaction_done_.Wait();
    }
